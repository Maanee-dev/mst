...
const ResortSlide = React.memo(({ resort, isLiked, isInBag, toggleLike, addItem, handleShare, openComments }: { 
  ...
  const isInView = useInView(slideRef, { margin: "100% 0px 100% 0px" });
  ...
  return (
    <div 
      ref={slideRef} 
      className={`w-full h-full flex-shrink-0 snap-center relative overflow-hidden ${!isInView ? 'pointer-events-none' : ''}`}
    >
      {isInView ? (
        <>
          {/* Horizontal Gallery */}
          ...
          >
            {images.map((img, idx) => {
              ...
              return (
                <div key={idx} className="w-full h-full flex-shrink-0 snap-center relative translate-z-0">
                  {isVisible && (
                    <img 
                      src={img} 
                      alt={`${resort.name} ${idx}`} 
          ...
          <AnimatePresence mode="wait">
            {activeIdx === 0 && (
                ...
                exit={{ opacity: 0, y: -20 }}
                className="absolute top-6 left-0 right-0 z-[505] flex items-center justify-between px-6 md:px-10 pointer-events-none will-change-transform"
              >
                {/* Profile Icon */}
                <button 
                  onClick={() => setIsUserPanelOpen(true)}
                  className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/10 backdrop-blur-md border border-white/10 flex items-center justify-center text-white pointer-events-auto hover:bg-white/20 transition-all"
                >
          ...
          <div className="absolute right-4 md:right-8 bottom-12 md:bottom-16 flex flex-col gap-5 items-center z-[506] will-change-transform">
            <button 
              onClick={() => toggleLike({ 
                id: resort.id, 
                name: resort.name, 
            ...
            >
              <div className={`w-11 h-11 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all backdrop-blur-md border border-white/10 ${isLiked(resort.id) ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-white/10 text-white hover:bg-white/20'}`}>
            ...
            <button 
              ...
              className="group flex flex-col items-center gap-1.5"
            >
              <div className="w-11 h-11 md:w-12 md:h-12 rounded-full bg-white/10 backdrop-blur-md text-white hover:bg-white/20 flex items-center justify-center transition-all border border-white/10 relative">
            ...
            <button 
              ...
              className="group flex flex-col items-center gap-1.5"
            >
              <div className={`w-11 h-11 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all backdrop-blur-md border border-white/10 overflow-hidden ${isInBag(resort.id) ? 'bg-sky-500 text-white shadow-[0_0_20px_rgba(14,165,233,0.4)]' : 'bg-white/10 text-white hover:bg-white/20'}`}>
                {isInBag(resort.id) ? (
                  <motion.div
            ...
            <button 
              ...
              className="group flex flex-col items-center gap-1.5"
            >
              <div className="w-11 h-11 md:w-12 md:h-12 rounded-full bg-white/10 backdrop-blur-md text-white hover:bg-white/20 flex items-center justify-center transition-all border border-white/10">
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* Close Button */}
        <div className="absolute top-6 right-6 md:right-10 z-[505] will-change-transform">
          <button 
            onClick={() => setDiscoveryMode(false)}
            className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-black/20 hover:bg-black/40 backdrop-blur-md rounded-full text-white transition-all group border border-white/10"
